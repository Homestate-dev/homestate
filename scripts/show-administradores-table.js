...
async function showAdministradoresTable() {
  ...
  try {
    ...
    // Obtener información de la tabla administradores
    const tableQuery = `
      SELECT 
        schemaname,
        tablename,
    ...
    if (tableResult.rows.length === 0) {
      const message = '❌ No se encontró la tabla administradores';
      console.log(message);
      output += message + '\n';
      return;
    }
   ...
   Esquema: ${table.schemaname}`;
    ...
    const constraintsQuery = `
      ...
      AND tc.table_name = 'administradores'
   ...
   • Total de restricciones: ${constraints.length}`;
    ...
    // Guardar en archivo
    const outputPath = path.join(__dirname, 'administradores.txt');
...
// Ejecutar el script
showAdministradoresTable().catch(console.error);
